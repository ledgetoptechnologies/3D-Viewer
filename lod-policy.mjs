...
// measured large shell can exceed that target. Allow a bounded expansion
...
// Using the raw Detail target here selects a full complex tree at Home and
...
export function lodCacheBudget(deviceMemoryGiB, memoryProfile = null) {
  ...
  if (Number.isFinite(memory) && memory <= 4) {
    ...
    // measured full-detail frontier for a large stress-test fixture is over
...
export function lodCacheMaxBytesForOverview(currentMaxBytes, overviewBytes, reduced = false) {
  ...
  // A large overview can exceed 1.25 GiB. A 1.75 GiB hard cap
