...
function createEphemeralMeasurementApi(repository,{config={},preflightRaster,validator}={}){
  ...
  router.get('/capabilities',async(req,res,next)=>{try{const access=await resolve(req),enabled=config.measurementCalculationsEnabled!==false,sources=enabled?sourceVersion(access).assets.filter(a=>['dsm','dtm'].includes(a.kind)&&/^(tif|tiff|geotiff)$/i.test(a.format||'')&&a.sha256).map(a=>({assetId:a.id,kind:a.kind,format:a.format,byteSize:a.byteSize,methods:['surface-cut-fill']})):[];
    res.json({capabilities:{personalPersistence:false,rasterCalculations:enabled,temporaryCalculations:true,serverCalculations:false},modelVersionId:access.model.activeVersion.id,calculationSources:sources,calculationMethods:sources.length?['surface-cut-fill']:[]});}catch(e){next(e);}});
  router.post('/calculations',async(req,res,next)=>{try{
    ...
    if(req.body.request?.method!=='surface-cut-fill')fail('measurement_method_unavailable',403);
    const request=validateCalculationRequest(req.body.request,measurement,sourceVersion(access));
    ...
    res.status(202).json({calculation:jobs.enqueue(access,pageHash,measurement,request)});
  }catch(e){next(e);}});
