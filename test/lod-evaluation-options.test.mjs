...
test('loading experiments require explicit independent per-page opt-in', () => {
  assert.deepEqual(lodEvaluationOptions(), { distanceDemand: false, loadingTiming: false });
  assert.deepEqual(lodEvaluationOptions('?lodDistanceDemand=1'), { distanceDemand: true, loadingTiming: false });
  assert.deepEqual(lodEvaluationOptions('?lodLoadingTiming=1'), { distanceDemand: false, loadingTiming: true });
  assert.deepEqual(lodEvaluationOptions('?lodDistanceDemand=1&lodLoadingTiming=1'), { distanceDemand: true, loadingTiming: true });
  for (const value of ['true', 'yes', '0', '-1', '100', 'NaN', '']) {
    assert.equal(lodEvaluationOptions('?lodDistanceDemand=' + value).distanceDemand, false);
  }
