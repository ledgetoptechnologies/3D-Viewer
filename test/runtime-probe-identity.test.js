...
test('accepts matching no-store probes at the exact source-derived schema', async () => {
  ...
  assert.equal(validateRuntimeProbeHeaders({ healthHeaders: headers(), readyHeaders: headers(), expectedSchemaVersion: 17, bakedRevision: valid.revision }), null);
});
...
for (const [name, mutate, expected] of [
  ...
  ['older schema', () => ({ healthHeaders: headers({ schema: '16' }), readyHeaders: headers({ schema: '16' }) }), /does not match expected schema version 17/],
  ['newer schema', () => ({ healthHeaders: headers({ schema: '18' }), readyHeaders: headers({ schema: '18' }) }), /does not match expected schema version 17/],
  ...
  const { validateRuntimeProbeHeaders } = await import('../scripts/runtime-probe-identity.mjs');
  const input = mutate();
  assert.match(validateRuntimeProbeHeaders({ ...input, expectedSchemaVersion: 17, bakedRevision: input.bakedRevision ?? valid.revision }), expected);
});
