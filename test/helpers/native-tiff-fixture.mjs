...
export function nativeTiffFixture({tiled=false,rowsPerStrip=1,width=2,height=2,tileWidth=2,tileLength=2,transform,encodedCountOverride,externalPadding=8192}={}) {
  const entries=[],tag=(id,type,values)=>entries.push({id,type,values:Array.isArray(values)?values:[values]}),sizes={3:2,4:4,12:8};
  ...
  tag(34735,3,[1,1,0,5,1024,0,1,1,1025,0,1,1,3072,0,1,32616,3076,0,1,9001,4099,0,1,9001]);
  ...
  for(let i=0;i<entries.length;i++){
    ...
    entry.values.forEach((v,n)=>{const at=(entry.offset||p+8)+n*sizes[entry.type];if(entry.type===3)output.writeUInt16LE(v,at);else if(entry.type===4)output.writeUInt32LE(v,at);else output.writeDoubleLE(v,at);});
  }
