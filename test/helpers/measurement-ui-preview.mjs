...
const records=new Map(seeds.map(r=>[r.id,r]));
...
const server=await createServer({root,configFile:false,appType:'custom',server:{host:'127.0.0.1',port:4471,strictPort:true},plugins:[{name:'synthetic-measurement-ui',configureServer(server){server.middlewares.use(async(req,res,next)=>{
...
await server.listen();console.log('Synthetic shipped measurement UI: http://127.0.0.1:4471');
