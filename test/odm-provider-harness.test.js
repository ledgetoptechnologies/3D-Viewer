...
async function fakeProvider({ loseFirstInitResponse=false, rejectRemove=false } = {}) {
  const tasks = new Map();
  const order = [];
  let lost = false;
  const server = http.createServer(async (request, response) => {
    ...
    if (request.method === 'GET' && /^\/task\/[^/]+\/download\/all\.zip$/.test(url.pathname)) { response.writeHead(200, { 'content-type':'application/zip' });response.end('zip');return; }
