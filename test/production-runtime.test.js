...
test('published image executes a real Obj2Tiles conversion and provenance audit', () => {
  ...
  assert.match(workflow, /EXPECTED_SCHEMA_VERSION: "28"/, 'published-image verification must match the current database schema');
});
...
test('production gates health/readiness and all routes behind exact proxy host and secret', async (t) => {
  ...
  assert.equal(ready.headers.get('x-ltds-viewer-schema-version'), '28');
  ...
  assert.equal(health.headers.get('x-ltds-viewer-schema-version'), '28');
