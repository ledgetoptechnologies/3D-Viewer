...
any sibling in the supplied root, and its JSON report omits the root path. All
production validation and disk-space preflight occurs before that child exists.

Production mode accepts only
`ghcr.io/ledgetoptechnologies/3d-viewer@sha256:<64 lowercase hex>` and compares
`--source-commit` with the read-only source identity baked into that runtime
image by CI. A mutable tag, missing build identity, or mismatch fails closed.
The space preflight uses bounded arithmetic and requires the generated byte
count plus the configured reserve plus an additional margin of the greater of
1 GiB or 5% of generated bytes.
...
The example explicitly retains a 10 GiB reserve in addition to the automatic
safety margin. Increase `--reserve-bytes` to match the operating storage policy;
never reduce it merely to force a preflight to pass.
...
pre-generation space calculation, a confirmed low-space refusal, and
`cleanup.verified=true`. Save that JSON with the release record. Do not claim
real-scale proof from the normal CI test or from a run on a different storage
class.
